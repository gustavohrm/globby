...
async function createUser(kv: KVNamespace): Promise<{ id: string; secret: string }> {
  const res = await handle(new Request("http://localhost/api/v1/users", { method: "POST" }), {
    USERS_KV: kv,
  } as unknown as Env);
...
describe("GET /api/v1/rooms", () => {
  it("returns empty rooms array initially", async () => {
    ...
    const res = await handle(new Request("http://localhost/api/v1/rooms"), makeEnv(kv, rooms, registry));
...
describe("GET /api/v1/rooms/:id", () => {
  it("returns the room when it exists", async () => {
    ...
    const res = await handle(new Request(`http://localhost/api/v1/rooms/${id}`), makeEnv(kv, rooms, registry));
  ...
  it("returns 404 for non-existent room", async () => {
    ...
    const res = await handle(new Request("http://localhost/api/v1/rooms/does-not-exist"), makeEnv(kv, rooms, registry));
...
describe("DELETE /api/v1/rooms/:id", () => {
  it("deletes the room and returns 204", async () => {
    ...
    const getRes = await handle(new Request(`http://localhost/api/v1/rooms/${id}`), makeEnv(kv, rooms, registry));
