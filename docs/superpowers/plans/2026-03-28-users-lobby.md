...
```ts
const ADJECTIVES = [
  "Silent", "Swift", "Brave", "Calm", "Dark", "Bright", "Wild", "Gentle",
  "Bold", "Clever", "Eager", "Fierce", "Glad", "Happy", "Icy", "Jolly",
  "Kind", "Lively", "Merry", "Noble", "Odd", "Proud", "Quick", "Rare",
  "Shy", "Tame", "Unique", "Vivid", "Warm", "Zany", "Ancient", "Blunt",
  "Crisp", "Daring", "Earnest", "Frosty", "Graceful", "Hollow", "Ironclad",
  "Jagged", "Keen", "Lunar", "Mystic", "Nimble", "Oblique", "Placid",
  "Quiet", "Rustic", "Stormy", "Tidy",
];

const NOUNS = [
  "Otter", "Fox", "Wolf", "Bear", "Hawk", "Raven", "Lynx", "Falcon",
  "Panda", "Tiger", "Crane", "Viper", "Moose", "Bison", "Heron", "Cobra",
  "Drake", "Eagle", "Finch", "Goose", "Hyena", "Ibis", "Jackal", "Kite",
  "Lemur", "Mink", "Newt", "Owl", "Puma", "Quail", "Robin", "Stoat",
  "Toad", "Urial", "Vole", "Wren", "Xerus", "Yak", "Zebra", "Adder",
  "Bison", "Coral", "Dingo", "Egret", "Ferret", "Gecko", "Hound", "Iguana",
  "Jaguar", "Koala",
];
...
describe("POST /api/v1/users", () => {
  it("creates a user and returns id, username, secret", async () => {
    ...
    const res = await handle(
      new Request("http://localhost/api/v1/users", { method: "POST" }),
      makeEnv(kv),
    );
...
register({
  ...
  handler: async (_req, _params, env) => {
    ...
    return Response.json(
      { id: user.id, username: user.username, secret: user.secret },
      { status: 201 },
    );
  },
});
...
describe("GET /api/v1/lobby/users", () => {
  it("returns empty array when no public users exist", async () => {
    const kv = makeMockKV();
    const res = await handle(
      new Request("http://localhost/api/v1/lobby/users"),
      makeEnv(kv),
    );
    expect(res.status).toBe(200);
